...
class ReactivateSubscriptionRouteTest {
    ...
    private val futureEnd = Instant.parse("2026-07-29T00:00:00Z")

    @Test
    fun `no scheduled non-renewal yields 404`() =
        testApplication {
            ...
            assertThat(resp.bodyAsText()).contains("errors/not-reactivatable")
        }

    @Test
    fun `pending cancellation with a running period is resumed and returns active`() =
        testApplication {
            ...
            assertThat(resp.bodyAsText()).contains("\"status\":\"active\"")
        }
    ...
    private fun ApplicationTestBuilder.install(
        ...
        routing { reactivateSubscriptionRoute(reactivate) }
    }
}
