...
# ADR-0080: regeneration-safe daily identity — fresh random UUID v7 per generation so a regenerated grid gets uncorrupted state (hint usage / progress / ADR-0075 sync blobs are keyed by puzzleId). No frontend change needed; superseded rows accumulate (GC deferred)
```
