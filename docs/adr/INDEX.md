...
ADR-0066  game/application/**/usecases/LobbyUseCases.kt   Amendment 2026-07-14 (d): JoinLobbyUseCase seats an authed join under the connect-time verified WhoAmI.displayName (never the client frame), mirroring REST-create; anon joins keep the client pseudonym
ADR-0066  game/api/src/main/kotlin/com/bliss/game/api/routes/LobbyWebSocketRoute.kt   Amendment 2026-07-14 (d): the WS route threads the verified WhoAmI.displayName into joinLobby so an authed seat shows the account name, not the client-sent guest pseudonym
ADR-0066  frontend/src/ui/routes/lobby.$lobbyId.tsx   Amendment 2026-07-14 (d): withLocalPlayer's rejoin fallback uses auth.whoami.displayName for authed users so the local pseudonym never flashes the guest name during the snapshot gap
ADR-0066  game/domain/src/main/kotlin/com/bliss/game/domain/Identifiers.kt   Amendment 2026-07-23 (e): new PlayerId identity (= userId when authed, else sessionId); derived at the WS edge from the server-verified userId, never a client frame
ADR-0066  game/domain/src/main/kotlin/com/bliss/game/domain/Lobby.kt   Amendment 2026-07-23 (e): Lobby.players re-keyed Map<PlayerId, Player>; GameSession.lockedPositions re-keyed Map<Position, PlayerId>; anon PlayerId == sessionId
ADR-0066  game/application/**/usecases/LobbyUseCases.kt   Amendment 2026-07-23 (e): JoinLobbyUseCase upserts idempotently by PlayerId (deletes the (b) seat-move); UpdateCellUseCase attributes locks to PlayerId; rebind re-keys roster + locks
ADR-0066  game/api/src/main/kotlin/com/bliss/game/api/routes/LobbyWebSocketRoute.kt   Amendment 2026-07-23 (e): edge derives PlayerId = verifiedUserId ?: sessionId; reconnect grace removes a player only when no live socket remains for its PlayerId; kick targets PlayerId
ADR-0066  game/api/asyncapi.yaml                    Amendment 2026-07-23 (e): PlayerId on Player/playerJoined/playerLeft/playerRenamed; lockedBy SessionId -> PlayerId (amends the ADR-0086 lockedBy binding)
ADR-0066  game/api/openapi.yaml                     Amendment 2026-07-23 (e): mirror the asyncapi lockedBy PlayerId retype on GET /v1/lobbies/{id} GameSession.lockedPositions
ADR-0066  game/infrastructure/**/db/migration/V4__lobby_players_player_id.sql   Amendment 2026-07-23 (e): expand-and-contract — lobby_players identity is coalesce(user_id, session_id); no row-dedup backfill (DELETE+INSERT full-rewrite); game_payload lockedBy read forward-compatibly
ADR-0066  frontend/src/application/game/playerScores.ts   Amendment 2026-07-23 (e): tallyValidatedLetters groups by PlayerId; roster/"you"/board-tint keyed on PlayerId; transport stays sessionId (no playerId-keyed self-echo suppression)
...
ADR-0089  */api/**/routes/PuzzleRoute.kt             Daily ETag = "<puzzleId>-<clue-content-hash>" (dailyEtagOf): id flips on regen (ADR-0081), hash flips on in-place clue correction (ADR-0108) so revalidation returns 200 not a stale 304 (2026-07-13 amendment)
# ADR-0089: amends ADR-0007 §4 (DNS-only posture narrowed to WS hosts); daily cache policy = anon-only public,max-age=0,must-revalidate + s-maxage-to-UTC-midnight + content ETag="<puzzleId>-<clue-hash>" (304), cookie ⇒ private,no-store; daily/list stays public,no-cache — never edge-cached (unbounded query variants vs exact-URL purge); regen propagates via ADR-0081 fresh-UUID + edge purge; in-place clue corrections (ADR-0108) propagate via the clue-hash ETag flip + edge purge (2026-07-13 amendment)
...
ADR-0095  grid/domain/src/main/kotlin/com/bliss/grid/domain/generation/LayoutAnchorer.kt  Low-density daily generation: anchored long runs (carve K targeted long horizontal runs, perturbation stays on so board stays fillable). Rejected LayoutDistiller (disables perturbation → unfillable). Daily config anchorCount=3 + best-of-N; depends on sigle short-word overlay. 2026-07-14 amendment: carveVertical (vertical anchor columns) exists but is default-off and NOT enabled in prod — measured worse on both boards (densifies, fewer long words)
ADR-0095  grid/domain/src/main/kotlin/com/bliss/grid/domain/generation/LongWordCoverage.kt  2026-07-14 amendment: per-axis grid-relative long-word thresholds (Lh=F*width, Lv=F*height, F=0.4) + letter-sum coverage score + bestByCoverage(N) selector. The lever that actually raises long-word count; best-of-N by coverage, NOT more anchoring
ADR-0095  grid/domain/src/main/kotlin/com/bliss/grid/domain/generation/GridConstraints.kt  Per-axis run caps lTargetHorizontal/lTargetVertical + anchorCount/anchorLength (default off; daily = 11/8, anchor 3). Do NOT re-add distillBudget (falsified: distiller disables perturbation). 2026-07-14: verticalAnchorCount/verticalAnchorLength default-off (measured-rejected, densifies)
ADR-0095  grid/application/src/main/kotlin/com/bliss/grid/application/puzzle/PuzzleConstraints.kt  Daily size split: dailyPuzzleConstraints() = 22×15 (re-scaled knobs anchor 3/len 10, lH 9/lV 6), distinct from defaultPuzzleConstraints() 28×20 API default. 2026-07-14: PUZZLE_BEST_OF_N=16 (on-demand parallel best-of-N by coverage, env-overridable PUZZLE_BEST_OF_N)
...
ADR-0114  frontend/src/application/progress/**       Solo progress is keyed by puzzleId alone (no grid fingerprint): pullAndMergeOne(puzzleId) pulls, semantically merges (ADR-0075), pushes back — no content-based discard. Supersedes ADR-0105
ADR-0114  frontend/src/infrastructure/session/localStorageSolo.ts  SoloStore bucket keyed by puzzleId; no fingerprint field or reconcile. Regen orphans progress via new puzzleId (ADR-0081); an in-place clue correction (ADR-0108) preserves it via the unchanged puzzleId
ADR-0114  frontend/src/ui/routes/play.tsx            /play loader calls pullAndMergeOne(puzzle.id) with no fingerprint; calendar (progressOf(summary.id)) and loader both key on puzzleId, so done-state and the opened grid cannot disagree
...
# ADR-0108: amendment 2026-07-24 (bulk correction seeding) — grid-worker --seed-corrections bulk-inserts clue_corrections replace rows (created_by=seed-job, exported_at pre-stamped so ExportCorrectionsUseCase's `exported_at IS NULL` skips the override flush), then the existing --process-corrections sweep backfills grids in place; reusable GA bulk clue-fix without regeneration/progress-orphaning. First user: ppas definition replacement. New binding paths (grid-worker --seed-corrections + a seed Job in the grid chart) land with the worker-command impl PR.
...
ADR-0108  grid/api/**/routes/CorrectionRoute*       GET /v1/corrections/preview: read-only dry-run counts (affectedDailies/affectedSolo) for a clue correction before it's applied; admin:signalements-gated
...
ADR-0110  grid/**/persistence/*GridBackfill*        Blocklist backfill is a NEW strategy (patch-only can't remove a word): match stored grids on payload wordText; dailies → EnsureUpcomingDailiesUseCase.execute(date,force=true); solo → DELETE row (regen on next GET). Progress orphaned by the new puzzleId (ADR-0081/0114)
...
ADR-0119  scripts/grid_family/build_derivational_family_edges.py  Folds `complexite=simple` Démonette relations to grid-cell A-Z lemma pairs (roadmap item 1, shipped); regenerate on a Démonette ingest bump
ADR-0119  grid/infrastructure/src/main/resources/morphology/derivational_family_edges.csv  Generated artifact (Démonette-derived simple relations); regenerate via scripts/grid_family/build_derivational_family_edges.py on a Démonette ingest bump
ADR-0119  grid/api/src/test/kotlin/com/bliss/grid/api/FamilyDedupGateTest.kt  Gate test asserting the derivational family edges block dedup for direct pairs (saut/sauter) without transitive spread (port/rapport)
ADR-0120  grid/infrastructure/src/main/kotlin/com/bliss/grid/infrastructure/persistence/CsvWordRepository.kt  Loads grammalecte-derived participle adj-lemma->verb-lemma edges (main resource, MPL-2.0) and ADR-0119's Démonette-derived simple derivational edges, and completes the per-surface family key so WordAcceptor bridges both; see ADR-0119 above for the derivational half
...
ADR-0120  scripts/grid_family/**                    Emits folded participle->verb family edges from the grammalecte lexique for the grid dedup bridge (ADR-0120); also home to ADR-0119's derivational edge builder (see above)
