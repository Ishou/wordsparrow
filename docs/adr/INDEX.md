...
ADR-0080  grid/application/src/main/kotlin/com/bliss/grid/application/puzzle/**  Daily-puzzle identity is unique-per-generation, not deterministic-from-date: DailyPuzzleSelector drops puzzleIdForDate; date resolves to the most-recently-created puzzles row for that puzzle_date; ListDailyPuzzles + EnsureUpcomingDailies idempotency switch from fixed-id get to date->current-row resolution (Wave 3b/3c). gridNumber/difficulty stay date-derived
ADR-0080  grid/api/src/main/kotlin/com/bliss/grid/api/routes/PuzzleRoute.kt  Daily GET (/v1/puzzles/daily ~line 111) resolves the date to its current row instead of computing the deterministic id; 404 unchanged when no row for the date (Wave 3b/3c)
ADR-0080  grid/*/src/main/resources/db/migration/**  Migration adds nullable puzzle_date column + index to puzzles; daily rows set it, on-demand path leaves it null; expand-and-contract, regeneration appends a new row (newest wins), no delete/update — preserves the immutable-puzzle design
# ADR-0080: regeneration-safe daily identity — fresh random UUID per generation so a regenerated grid gets uncorrupted state (hint usage / progress / ADR-0075 sync blobs are keyed by puzzleId). No frontend change needed; superseded rows accumulate (GC deferred)
```
