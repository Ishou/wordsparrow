...
dependencies — a GPL-licensed npm or Maven dependency bundled into
our shipped Docker image carries the same commercial-incompatibility
risk as a CC BY-NC training source. GPL-2.0 is included alongside
GPL-3.0 variants: both are fully viral copyleft with the same
commercial-use incompatibility; the initial draft omitted GPL-2.0
and `GPL-2.0-or-later`, leaving a real gap that was closed in
PR #762.
