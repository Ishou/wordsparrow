...
- **Ready to merge.** All blocking checks `SUCCESS` (`build`, `commitlint`, `branch-name`, `dco`, `gitleaks`, `dependency-review`, `regen-and-diff`, `spectral`) AND `mergeable` AND `mergeStateStatus != BLOCKED` AND the latest §6a review's first line is `LGTM` (case-insensitive). → `gh pr merge <pr#> --squash` (no `--delete-branch`). Log `MERGED`.
