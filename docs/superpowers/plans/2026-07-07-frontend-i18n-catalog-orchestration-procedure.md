...
| P4a chrome | `refactor/i18n-components-shell` | `main` | `refactor(frontend): i18n layout/brand/primitives components` | Convert `frontend/src/ui/components/{layout,brand,primitives,decorations,keyboard,compte,tour}/**` + `PrivacyNotice.tsx`. (P4 was split — the `ui/components` subdirs carry ~30 files of literals, over the target as one PR.) |
| P4b grid | `refactor/i18n-components-grid` | `main` | `refactor(frontend): i18n grid components` | Convert `frontend/src/ui/components/grid/**` (~7 files). Uncontrolled-input sensitive (ADR-0002 §4) — swap only string props. |
| P4c sondage | `refactor/i18n-components-sondage` | `main` | `refactor(frontend): i18n sondage components` | Convert `frontend/src/ui/components/sondage/**` (~10 files, survey UI). |
| P5a v2-multiplayer | `refactor/i18n-v2-multiplayer` | `main` | `refactor(frontend): i18n v2 multiplayer` | `frontend/src/ui/v2/multiplayer/**` (5 files). (P5 split — v2 has 30 files of literals, over target as one PR.) |
| P5b v2-subscription | `refactor/i18n-v2-subscription` | `main` | `refactor(frontend): i18n v2 subscription screens` | v2 billing/subscription: `Abonnement*`, `AbonnementSection`, `ConditionsAbonnementScreen`, `ReceiptsSection`, `UpsellEntries` (~7 files; ConditionsAbonnement is CGV legal prose — defer to a follow-up if bilingual/oversized like PrivacyNotice). |
| P5c-1 v2-account | `refactor/i18n-v2-account-legal` | `main` | `refactor(frontend): i18n v2 account + settings screens` | v2 account/settings: `CompteScreen`, `ReglagesScreen`, `ConnexionScreen`, `EraseData`. (P5c split — 8 files over target.) |
| P5c-2 v2-legal | `refactor/i18n-v2-legal-info` | `main` | `refactor(frontend): i18n v2 legal + info screens` | v2 legal/info (confirmed French-only prose): `AProposScreen`, `AideScreen`, `ConfidentialiteScreen`, `MentionsLegalesScreen`. |
| P5d v2-shell | `refactor/i18n-v2-shell` | `main` | `refactor(frontend): i18n v2 shell + nav screens` | v2 shell/nav/grilles: `BottomNav`, `DesktopAppBar`, `MobileTopBar`, `MenuSheet`, `DailyCalendar`, `GrillesArchiveScreen`, `GrillesEmptyState`, `LoaderRetry`, `SparrowScenes`, `UpdatePrompt` (~10 files). |
| P6a routes+seo | `refactor/i18n-routes-seo-app` | `main` | `refactor(frontend): i18n routes + seo` | Convert `frontend/src/ui/routes/**` and `frontend/src/ui/seo/**` (~11 files, incl. SEO `<title>`/meta). (P6 split — the application-layer refactor is architecturally distinct.) |
| P6b application | `refactor/i18n-application-codes` | `main` | `refactor(frontend): map application error copy at the ui edge` | `frontend/src/application/**` user-facing French strings (e.g. `application/errors/messageForApiError.ts`, and any in `AuthClient`/`LobbyClient`/`BillingClient`). The `application` layer cannot import `ui/i18n`, so expose **codes/keys** from application and map them to copy via `t()` at the `ui` edge; update `messageForApiError.test.ts` accordingly. FINAL phase. |
