    ...
    steps:
      ...
      - name: Find bumped Chart.yaml + build bundle
        ...
        run: |
          ...
          COUNT=$(python -c "import json;print(len(json.load(open('/tmp/bumps.json'))))")
          if [ "$COUNT" != "1" ]; then
            echo "::warning::expected 1 bump, got $COUNT; skipping"
            echo "skip=true" >> "$GITHUB_OUTPUT"; exit 0
          fi
          python -c "import json;print(json.dumps(json.load(open('/tmp/bumps.json'))[0]))" > /tmp/bump.json
          NAME=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['name'])")
          OLD=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['old'])")
          NEW=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['new'])")
          CHART_DIR=$(dirname "$FILE")
          REPO=$(python -c "import yaml,sys;d=yaml.safe_load(open('$FILE'));print(next(x['repository'] for x in d['dependencies'] if x['name']=='$NAME'))")
...
```bash
          ...
          python -c "import json;print(json.dumps(json.load(open('/tmp/bumps.json'))[0]))" > /tmp/bump.json
          MODE=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['mode'])")
          if [ "$MODE" = "A" ]; then
            NAME=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['name'])")
            OLD=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['old'])")
            NEW=$(python -c "import json;print(json.load(open('/tmp/bump.json'))['new'])")
            CHART_DIR=$(dirname "$FILE")
            REPO=$(python -c "import yaml;d=yaml.safe_load(open('$FILE'));print(next(x['repository'] for x in d['dependencies'] if x['name']=='$NAME'))")
            helm repo add up "$REPO" >/dev/null 2>&1 || helm repo add up "$REPO"
            helm repo update up >/dev/null
            helm show values "up/$NAME" --version "$OLD" > /tmp/vold.yaml || : > /tmp/vold.yaml
            helm show values "up/$NAME" --version "$NEW" > /tmp/vnew.yaml || : > /tmp/vnew.yaml
