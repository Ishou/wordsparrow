...
def _implicit_relative_verb(
    ...
    Heads that also read as a noun are ambiguous (`Porte le glaive` vs `Porte
    d'entrée`) and stay with the ranker unless `authored_head_pos` settles it —
    the author knows which they wrote, and no tagger recovers that as reliably."""
    if target_pos not in ("nom", "adj"):
        return None
