...
const pa = a as unknown as PlayerId;
const pb = b as unknown as PlayerId;
const players: ReadonlyArray<Player> = [
  { playerId: pa, sessionId: a, pseudonym: 'Alex' as Pseudonym, joinedAt: '2026-06-27T15:30:00Z' as Instant },
  { playerId: pb, sessionId: b, pseudonym: 'Sam' as Pseudonym, joinedAt: '2026-06-27T15:31:00Z' as Instant },
];
const empty = new Set<SessionId>();

function renderStrip(scores?: ReadonlyMap<PlayerId, number>) {
  return render(
    <PlayerStrip
      players={players}
      currentPlayerId={pa}
...
describe('PlayerStrip score', () => {
  it('renders each player validated-letter count', () => {
    renderStrip(new Map([[pa, 12], [pb, 5]]));
  ...
  it('does not reorder chips by score', () => {
    renderStrip(new Map([[pa, 1], [pb, 99]]));
