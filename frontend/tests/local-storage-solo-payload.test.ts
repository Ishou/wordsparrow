...
describe('localStorageSolo grid fingerprint (ADR-0105 stale-grid guard)', () => {
  const FP = 'grid-aaa';
  const OTHER_FP = 'grid-bbb';

  beforeEach(() => window.localStorage.clear());
  afterEach(() => window.localStorage.clear());

  it('discards progress typed on a different grid and stamps the new fingerprint', async () => {
    const { saveSoloLetter, saveSoloLockedCell, reconcileSoloFingerprint, loadSoloPayload } =
      await loadFresh();
    saveSoloLetter(SESSION, PUZZLE_A, 0, 0, 'X');
    saveSoloLockedCell(SESSION, PUZZLE_A, 0, 0);
    reconcileSoloFingerprint(SESSION, PUZZLE_A, FP); // first stamp — nothing to discard yet
    reconcileSoloFingerprint(SESSION, PUZZLE_A, OTHER_FP); // grid regenerated ⇒ discard

    const after = loadSoloPayload(SESSION, PUZZLE_A);
    expect(after.entries).toEqual([]);
    expect(after.lockedCells).toEqual([]);
    expect(after.fingerprint).toBe(OTHER_FP);
  });

  it('treats a legacy blob (no stored fingerprint) as stale and discards it', async () => {
    const { saveSoloLetter, reconcileSoloFingerprint, loadSoloPayload } = await loadFresh();
    saveSoloLetter(SESSION, PUZZLE_A, 1, 1, 'Z'); // written before fingerprints existed → no fp
    reconcileSoloFingerprint(SESSION, PUZZLE_A, FP);
    expect(loadSoloPayload(SESSION, PUZZLE_A).entries).toEqual([]);
  });

  it('keeps progress whose fingerprint matches the current grid', async () => {
    const { saveSoloLetter, reconcileSoloFingerprint, loadSoloPayload } = await loadFresh();
    reconcileSoloFingerprint(SESSION, PUZZLE_A, FP); // stamp before play
    saveSoloLetter(SESSION, PUZZLE_A, 2, 2, 'S'); // preserves stamped fp
    reconcileSoloFingerprint(SESSION, PUZZLE_A, FP); // same grid on reload ⇒ keep

    expect(loadSoloPayload(SESSION, PUZZLE_A).entries).toContainEqual({ r: 2, c: 2, l: 'S' });
    expect(loadSoloPayload(SESSION, PUZZLE_A).fingerprint).toBe(FP);
  });

  it('carries the fingerprint through the wire payload and replace', async () => {
    const { replaceSoloPayload, loadSoloPayload } = await loadFresh();
    replaceSoloPayload(SESSION, PUZZLE_A, {
      entries: [{ r: 0, c: 0, l: 'A' }],
      lockedCells: [],
  ...
  it('retains a bare fingerprint stamp on an unplayed puzzle without listing it as progress', async () => {
    const { reconcileSoloFingerprint, loadSoloPayload, listSoloPuzzleIds } = await loadFresh();
    reconcileSoloFingerprint(SESSION, PUZZLE_A, FP);
    // Stamp survives (so the next write is tagged) even though there is no progress yet.
    expect(loadSoloPayload(SESSION, PUZZLE_A).fingerprint).toBe(FP);
    expect(loadSoloPayload(SESSION, PUZZLE_A).entries).toEqual([]);
    expect(listSoloPuzzleIds(SESSION)).toEqual([PUZZLE_A]);
  });
