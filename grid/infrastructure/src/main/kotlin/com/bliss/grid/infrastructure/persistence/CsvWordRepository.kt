...
class CsvWordRepository(
    ...
    companion object {
        ...
        // Folded adj-participle-lemma -> verb-lemma edges (grammalecte-derived, ADR-0120); a main
        // resource is fine here — grammalecte's lexicon carries no ADR-0058 redistribution restriction.
        private const val PARTICIPLE_FAMILY_EDGES_PATH = "/morphology/participle_family_edges.csv"

        // Folded simple-derivation lemma pairs (Démonette-2-derived, ADR-0119 item 1). ADR-0058
        // forbids redistributing Démonette-2 content, so this is NOT a main resource: it loads
        // through the same private corpus-dir resolver as words-fr.csv (ADR-0097), never the
        // classpath in production. Missing file (not yet published to the corpus dir) degrades
        // to no derivational bridging rather than failing the load.
        private const val DERIVATIONAL_FAMILY_EDGES_PATH = "/morphology/derivational_family_edges.csv"

        private fun loadFamilyEdges(
            path: String,
            resolver: (String) -> java.io.InputStream?,
        ): Map<String, Set<String>> {
            val edges = HashMap<String, MutableSet<String>>()
            resolver(path)?.bufferedReader(StandardCharsets.UTF_8)?.useLines { lines ->
                lines.drop(1).forEach { line ->
                    val parts = line.split(",")
                    if (parts.size == 2) edges.getOrPut(parts[0]) { HashSet() }.add(parts[1])
                }
            }
            return edges
        }

        private val PARTICIPLE_FAMILY_EDGES: Map<String, Set<String>> by lazy {
            loadFamilyEdges(PARTICIPLE_FAMILY_EDGES_PATH, CLASSPATH_RESOLVER)
        }
            ...
            return stream.use { input ->
                InputStreamReader(input, StandardCharsets.UTF_8).use { reader ->
                    ...
                    CSVParser.parse(reader, format).use { parser ->
                        ...
                        // Bridge morphological families the per-surface lemma misses: grammalecte participles severed from their verb (émanée→émaner) and Démonette simple derivations (saut→sauter, race→raciste). Directed edges block edge-adjacent pairs without transitive spread; only bridge to lemmas actually present in this corpus (no phantom tokens).
                        val familyEdges = HashMap<String, MutableSet<String>>()
                        for ((k, v) in PARTICIPLE_FAMILY_EDGES) familyEdges.getOrPut(k) { HashSet() } += v
                        for ((k, v) in loadFamilyEdges(DERIVATIONAL_FAMILY_EDGES_PATH, resolver)) {
                            familyEdges.getOrPut(k) { HashSet() } += v
                        }
                        val corpusLemmas = lemmasBySurface.values.flatMapTo(HashSet()) { it }
                        for (lemmas in lemmasBySurface.values) {
                            lemmas +=
                                lemmas.flatMap { familyEdges[it].orEmpty() }.filter { it in corpusLemmas }
                        }
