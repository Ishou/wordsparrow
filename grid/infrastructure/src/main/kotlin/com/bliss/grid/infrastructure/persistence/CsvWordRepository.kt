...
class CsvWordRepository(
    ...
    companion object {
        ...
        // Folded lemma family-union edges: grammalecte participle->verb (ADR-0120) + Démonette simple derivational relations (ADR-0119 item 1). Regenerated by the scripts/grid_family builders.
        private val FAMILY_EDGE_PATHS =
            listOf("/morphology/participle_family_edges.csv", "/morphology/derivational_family_edges.csv")

        private val FAMILY_EDGES: Map<String, Set<String>> by lazy {
            val edges = HashMap<String, MutableSet<String>>()
            for (path in FAMILY_EDGE_PATHS) {
                CLASSPATH_RESOLVER(path)?.bufferedReader(StandardCharsets.UTF_8)?.useLines { lines ->
                ...
                InputStreamReader(input, StandardCharsets.UTF_8).use { reader ->
                    ...
                    CSVParser.parse(reader, format).use { parser ->
                        ...
                        for (lemmas in lemmasBySurface.values) {
                            lemmas +=
                                lemmas.flatMap { FAMILY_EDGES[it].orEmpty() }.filter { it in corpusLemmas }
                        }
