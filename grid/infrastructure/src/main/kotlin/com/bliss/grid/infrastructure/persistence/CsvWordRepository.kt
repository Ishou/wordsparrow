...
class CsvWordRepository(
    ...
    companion object {
        ...
        // Folded adj-participle-lemma -> verb-lemma edges (grammalecte-derived, ADR-0120); regenerated by scripts/grid_family/build_participle_family_edges.py.
        ...
        private val PARTICIPLE_FAMILY_EDGES: Map<String, Set<String>> by lazy {
            ...
            CLASSPATH_RESOLVER(PARTICIPLE_FAMILY_EDGES_PATH)?.bufferedReader(StandardCharsets.UTF_8)?.useLines { lines ->
            ...
            return stream.use { input ->
                InputStreamReader(input, StandardCharsets.UTF_8).use { reader ->
                    ...
                    CSVParser.parse(reader, format).use { parser ->
                        ...
                        // Bridge participle families: grammalecte tags a participle as BOTH a verb (ppas/ppre) and an adjective with its own lemma, and the corpus keeps one lemma per surface — so inflected participle-adjectives (émanée→émané) are severed from their verb (émaner) for dedup. Add the verb lemma from the grammalecte-derived edges.
                        for (lemmas in lemmasBySurface.values) lemmas += lemmas.flatMap { PARTICIPLE_FAMILY_EDGES[it].orEmpty() }
